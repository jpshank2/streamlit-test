...
def level_1_wip(st):
    try:
        ...
        with py_col:
            ...
            metric_three.metric('Billable Hours', '{:,.2f}'.format(my_py_util['billable_hours']))
            metric_four.metric('Nonbillable Hours', '{:,.2f}'.format(my_py_util['non_bill_hours']))
            metric_two.metric('Utilization', '{:,.2f}%'.format(my_py_util['utilization']))
