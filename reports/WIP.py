...
def level_1_wip(st):
    try:
        ...
        with cy_col:
            ...
            metric_util_one, metric_util_two = st.columns(2) 
            metric_util_three, metric_util_four = st.columns(2)
            metric_util_one.metric('Total Hours', '{:,.2f}'.format(my_cy_util['total_hours']), (my_cy_util['total_hours'] - my_py_util['total_hours']))
            metric_util_three.metric('Billable Hours', '{:,.2f}'.format(my_cy_util['billable_hours']), (my_cy_util['billable_hours'] - my_py_util['billable_hours']))
            metric_util_four.metric('Nonbillable Hours', '{:,.2f}'.format(my_cy_util['non_bill_hours']), (my_cy_util['non_bill_hours'] - my_py_util['non_bill_hours']))
            metric_util_two.metric('Utilization', '{:,.2f}%'.format(my_cy_util['utilization']), (my_cy_util['utilization'] - my_py_util['utilization']))

            st.markdown('##### Current Year Effective Rate and Realization')
            my_cy_real = my_realization(cy_wip_df)
            metric_real_one, metric_real_two = st.columns(2) 
            metric_real_three, metric_real_four = st.columns(2)
            metric_real_one.metric('WIP Amount', '${:,.2f}'.format(my_cy_real['wip_amount']), (my_cy_real['wip_amount'] - my_py_real['wip_amount']))
            metric_real_two.metric('WIP Billed', '${:,.2f}'.format(my_cy_real['wip_billed']), (my_cy_real['wip_billed'] - my_py_real['wip_billed']))
            metric_real_three.metric('Effective Rate', '${:,.2f}'.format(my_cy_real['eff_rate']), (my_cy_real['eff_rate'] - my_py_real['eff_rate']))
            metric_real_four.metric('Realization', '{:,.2f}%'.format(my_cy_real['realization']), (my_cy_real['realization'] - my_py_real['realization']))
