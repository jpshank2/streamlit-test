...
def level_1_wip(st):
    try:
        ...
        py_real.metric('Avg Realization for Level PY', '{:.2f}%'.format(py_benchmark_real), round(round((py_real_df['WIP_BILLED'] / py_real_df['WIP_AMOUNT']) * 100, 2) - py_benchmark_real, 2))
        py_eff.metric('Avg Effective Rate for Level PY', '${:,.2f}%'.format(py_benchmark_eff), round(round((py_real_df['WIP_BILLED'] / py_real_df['BILLABLE_HOURS']), 2) - py_benchmark_eff, 2))
