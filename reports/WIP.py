...
def level_1_wip(st):
    try:
        ...
        cy_wip_service_fig = pie(cy_wip_service_df, values='WIP_HOURS', names='SERVICETITLE', title='CY WIP Hours by Service Title').update_layout(pie_style)
        ...
        py_wip_service_fig = pie(py_wip_service_df, values='WIP_HOURS', names='SERVICETITLE', title='PY WIP Hours by Service Title').update_layout(pie_style)
...
def level_4_wip(st):
    try:
        ...
        static_five.metric(label='Target < 5%', value=overdue_wip, delta=('% WIP over 90 Days' if overdue_wip < 5 else '-% WIP over 90 Days'))
        ...
        partner_fig = bar(partner_df, x='OUTSTANDING_WIP', y=partner_y_val, orientation='h', barmode='group', title='Firm WIP by Client Partner', text='OUTSTANDING_WIP').update_layout(h_bar_style).update_traces(texttemplate="%{OUTSTANDING_WIP:$.2f}")
