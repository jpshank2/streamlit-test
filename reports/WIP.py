...
from TRANS_WIP WIP
    ...
    LEFT JOIN CLIENT_BMSS_SANDBOX_DB.ANONYMOUS.DIM_CLIENT_ANONymous A ON C.CONTINDEX = A.CONTIDX AND A.NAME IS NOT NULL
    INNER JOIN DIM_DATES D ON D.CALENDAR_DATE = WIP.WIPDATE
    INNER JOIN CLIENT_BMSS_SANDBOX_DB.PE.DIM_ANON_STAFF S ON S.STAFFINDEX = WIP.STAFFINDEX
    LEFT JOIN CLIENT_BMSS_SANDBOX_DB.PE.DIM_ANON_STAFF CP ON CP.STAFFINDEX = C.CLIENT_PARTNER_IDX
WHERE TRANSTYPE = 1
