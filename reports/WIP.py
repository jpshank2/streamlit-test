...
def level_1_wip(st):
    try:
        ...
        with py_col:
            ...
            metric_one.metric('Total Hours', '{:,.2f}'.format(my_py_util['total_hours']), ' ', 'off')
            metric_three.metric('Billable Hours', '{:,.2f}'.format(my_py_util['billable_hours']), ' ', 'off')
            metric_four.metric('Nonbillable Hours', '{:,.2f}'.format(my_py_util['non_bill_hours']), ' ', 'off')
            metric_two.metric('Utilization', '{:,.2f}%'.format(my_py_util['utilization']), '', 'off')

            st.markdown('##### Prior Year Effective Rate and Realization')
            my_py_real = my_realization(py_wip_df)
            metric_one.metric('WIP Amount', '${:,.2f}'.format(my_py_real['wip_amount']), ' ', 'off')
            metric_two.metric('WIP Billed', '${:,.2f}'.format(my_py_real['wip_billed']), ' ', 'off')
            metric_three.metric('Effective Rate', '${:,.2f}'.format(my_py_real['eff_rate']), ' ', 'off')
            metric_four.metric('Realization', '{:,.2f}%'.format(my_py_real['realization']), ' ', 'off')
